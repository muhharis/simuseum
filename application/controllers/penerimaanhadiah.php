...
class Penerimaanhadiah extends CI_Controller {
	...
	function tambah(){
		...
		$isi['kodeunik2'] = $this->master_model->buat_kode_penerimaan_hadiah();

		$isi['addhd'] = $get_hd->result();
		$isi['no_penerimaan_hadiah'] = '';
		$isi['tanggal']	    = '';
		$isi['jam'] 	= '';
		$isi['jumlah'] = '';
		$isi['ket']	    = '';
	...
	function ubah(){
		...
		$query = $this->db->get('in_penerimaan_hadiah');
		if($query->num_rows()>0)
		{
			...
			{	
				...
				$isi['no_penerimaan_hadiah'] = $row->no_penerimaan_hadiah;
	...
	function simpan(){
		...
		$key = $this->input->post('no_penerimaan_hadiah');
		$data_penha['no_penerimaan_hadiah'] = $this->input->post('no_penerimaan_hadiah');
		$data_penha['tanggal'] = $this->input->post('tanggal');
		...
		else 
		{
			...
			Redirect('penerimaanhadiah/tambah');
		}
